      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {photos.map((photo) => (
          ...
          >
            ...
            <div className="truncate px-2 py-1 text-xs text-slate-500">{photo.name}</div>
          </a>
        ))}
      </div>
    </div>
...
export default function ReturnRecordApp() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      photos: records.reduce(
        (sum, r) => sum + (r.invoicePhotos?.length ?? 0) + (r.productPhotos?.length ?? 0),
        0
      ),
    };
  }, [records]);
  ...
  const removePhoto = (type, id) => {
    if (type === "invoice") setInvoicePhotos((prev) => prev.filter((p) => p.id !== id));
    if (type === "product") setProductPhotos((prev) => prev.filter((p) => p.id !== id));
  };
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        >
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <h1 className="text-3xl font-bold tracking-tight">3종 반품 검사/수리 기록 프로그램</h1>
              <p className="mt-2 text-sm text-slate-600">
                대상: 휴대용분유포트 · 분유쉐이커 · LED분유쉐이커 / 일반반품 · 변심반품
              </p>
              <p className="mt-1 text-sm text-slate-500">
                링크맘에서 검사 및 수리 후 정상화 제품과 불량 제품을 구분하고, 송장 사진 중심으로 기록을 남기는 용도입니다.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" onClick={() => downloadCSV(records, "반품기록.csv")}> 
                <Download className="mr-2 h-4 w-4" /> CSV 내보내기
              </Button>
              <Button variant="outline" onClick={() => downloadJSON(records, "반품기록백업.json")}>
                <Download className="mr-2 h-4 w-4" /> 백업 저장
              </Button>
            </div>
          </div>
        </motion.div>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <StatCard title="전체 기록" value={stats.total} subtitle="누적 등록 건수" icon={Package} />
          <StatCard title="정상화 완료" value={stats.normal} subtitle="안성 물류 재고추가 대상" icon={Wrench} />
          <StatCard title="불량 판정" value={stats.bad} subtitle="안성 물류 폐기 대상" icon={XCircle} />
          <StatCard title="사진 수" value={stats.photos} subtitle="송장/제품 사진 합계" icon={Camera} />
        </div>
                    ...
                    <div className="space-y-3 rounded-2xl border p-4">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">주문번호 대신 송장 사진으로 기록</p>
                        </div>
                        <Label htmlFor="invoice-upload" className="cursor-pointer">
                          <div className="inline-flex items-center rounded-2xl border px-3 py-2 text-sm">
                            <ImagePlus className="mr-2 h-4 w-4" /> 업로드
                          </div>
                        </Label>
                      </div>
                      <Input
                      ...
                      {!!invoicePhotos.length && (
                        <div className="grid grid-cols-2 gap-3">
                          {invoicePhotos.map((photo) => (
                            <div key={photo.id} className="relative overflow-hidden rounded-2xl border">
                              <img src={photo.dataUrl} alt={photo.name} className="h-28 w-full object-cover" />
                              <button
                    ...
                    <div className="space-y-3 rounded-2xl border p-4">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">필요 시 외관/불량 상태 함께 저장</p>
                        </div>
                          ...
                          <CardContent className="p-5">
                            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                              <div className="space-y-3">
                                ...
                                <p className="text-sm text-slate-500">등록일시: {formatDateTime(record.createdAt)}</p>
                                {record.memo && <p className="text-sm leading-6 text-slate-700">{record.memo}</p>}
                              </div>
                              <div className="flex gap-2">
                                <Dialog>
                                  <DialogTrigger asChild>
                                    <Button variant="outline" className="rounded-2xl">상세보기</Button>
                                  </DialogTrigger>
                                  <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-4xl rounded-3xl">
                                    <DialogHeader>
                                      <DialogTitle>{record.product} / {record.returnType}</DialogTitle>
                                    </DialogHeader>
                                    <div className="space-y-5 text-sm">
                                      <div className="grid gap-3 md:grid-cols-3">
                                        <div className="rounded-2xl border p-3"><span className="text-slate-500">검사결과</span><div className="mt-1 font-medium">{record.result}</div></div>
                                        <div className="rounded-2xl border p-3"><span className="text-slate-500">송장 사진</span><div className="mt-1 font-medium">{record.invoicePhotos?.length ?? 0}장</div></div>
                                        <div className="rounded-2xl border p-3"><span className="text-slate-500">제품 사진</span><div className="mt-1 font-medium">{record.productPhotos?.length ?? 0}장</div></div>
                                      </div>
                                      {record.memo && (
                                        <div className="rounded-2xl border p-4">
                                          <p className="mb-2 font-medium">비고</p>
                                          <p className="whitespace-pre-wrap leading-6 text-slate-700">{record.memo}</p>
                                        </div>
                                      )}
                                      <PhotoGrid title="송장 사진" photos={record.invoicePhotos} />
                                      <PhotoGrid title="제품 사진" photos={record.productPhotos} />
                                    </div>
                                  </DialogContent>
                                </Dialog>
                                <Button
